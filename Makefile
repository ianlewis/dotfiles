...
.PHONY: e2e-test
e2e-test: bats-e2e tmux-config-test nvim-checkhealth ## Run all end-to-end tests.
...
tmux-config-test: $(E2E_HOME)/.installed ## Test tmux config for parsing errors (e2e).
	@# bash \
	# Check tmux config for parsing errors. This needs to be an e2e test \
	# since it relies on the home directory paths. \
	HOME="$${E2E_HOME}" \
		tmux -f $(REPO_ROOT)/tmux/_tmux.conf start-server \; source-file -n $(REPO_ROOT)/tmux/_tmux.conf
